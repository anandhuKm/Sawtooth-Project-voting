...
const subscribe = (stream) => {
    ...
    const voterSub = getVoterUploadActionSubscription();
    const clientSubscriptionRequest = 
      getClientSubscriptionRequest([blockSub, deltaSub, actionSub, voterSub], [NULL_BLOCK_ID]);
  ...
  const getCookieJarActionSubscription = () => {
    return EventSubscription.create({
      eventType: 'voting/voting-action'
    })
  }


  const getVoterUploadActionSubscription = () => {
    return EventSubscription.create({
      eventType: 'voting/voter-upload'
    })
  }
